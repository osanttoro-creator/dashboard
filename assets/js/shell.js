...
(function (global) {
  ...
  Shell.wireCelular = function () {
    ...
    if (menu && botao) {
      /* ========================================================
         O ESTADO DA FOLHA: UMA CLASSE, NÃO `hidden`
         --------------------------------------------------------
         Com `hidden` a folha ficava em `display: none` entre uma
         abertura e outra, e o navegador precisava calcular o
         layout dos 95 nós de novo a cada toque. Medido no site no
         ar, num celular médio (processador freado 6×): de 1,0 a
         1,9 SEGUNDO até a folha aparecer, num único layout de
         1,6s — o documento tem doze mil nós, e uma caixa nova
         obriga a medir tudo o que está vivo.

         Agora, depois que o aparelho fica ocioso, a folha sai do
         `display: none` e passa a viver medida e invisível. Abrir
         é trocar `visibility`, e isso não remede nada.

         `inert` é o que substitui `hidden` no que importa: a
         folha fechada continua fora da ordem de Tab e fora da
         árvore de acessibilidade. Navegador sem `inert` cai no
         `visibility: hidden` do CSS, que faz o mesmo.
         ======================================================== */
      const estaAberta = () => menu.classList.contains('esta-aberta');
      ...
      botao.addEventListener('click', (ev) => {
        ev.stopPropagation();
        if (estaAberta()) fechar(true); else abrir();
      });
      ...
      document.addEventListener('keydown', (ev) => {
        if (ev.key === 'Escape' && estaAberta()) { ev.preventDefault(); fechar(true); }
      });
